...
half4 ToonLight(InputData inputData, half3 diffuse, half4 specularGloss, half smoothness, half alpha)
{
	...
	float3 toonRamp = SAMPLE_TEXTURE2D(_ToonRamp, sampler_ToonRamp, halfLambert);
	
	float3 diffuseColor = inputData.bakedGI + (toonRamp * attenuatedLightColor); 
	...
	finalColor = (diffuseColor * diffuse) + specularToon;
